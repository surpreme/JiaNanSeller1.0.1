 ...
 * @description:
 */
public abstract class BaseObserver3<T> implements Observer<BaseData3<T>> {

    protected Context context;
    private String labelTxt;

    public BaseObserver3(Context mContext, String labelTxt) {
    ...
    public BaseObserver3(Context mContext) {
        this.context = mContext;
    }
    ...
    //获取数据
    @Override
    public void onNext(BaseData3<T> value) {

        try {
            if (value.isSuccess()) {
                Class<T> tClass = (Class<T>) ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
                T ordersListBean = BeanConvertor.convertBean(value.getDatas(), tClass);
                onSuccees(ordersListBean, value.isHasmore(), value.getPage_total());
            } else {
                ...
                if (value.getLogin() != null && value.getLogin().equals("0") || value.getToken_expired() != null && value.getToken_expired().equals("true")) {
                    PopupWindowsUtils.getmInstance().showDialogPopupWindow(
                            ...
                            v -> {
                                ActivityManager.getInstance().getCurrentActivity().startActivity(
                                        new Intent(ActivityManager.getInstance().getCurrentActivity(), LoginActivity.class));
                                PopupWindowsUtils.getmInstance().dismissPopWindow();
                            });
...
//    protected abstract void onSuccees(T t) throws Exception;
    protected abstract void onSuccees(T t, boolean hasmore, int maxPage) throws Exception;
